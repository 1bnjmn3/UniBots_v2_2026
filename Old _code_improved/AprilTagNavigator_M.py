...
class AprilTagNavigator:
    ...
    def process_frame(self, frame):
        ...
        for tag in results:
            print(f"DEBUG: Camera physically saw Tag ID: {tag.tag_id}")
            ...
            # If the tag is too far, the 3D pose might be in a flipped local minimum.
            # Rely on 2D pixel offset instead of 3D yaw/lateral.
            # Extract your camera's true optical center and focal length from camera_params
            # camera_params is passed into __init__ as (fx, fy, cx, cy)
            fx = self.camera_params[0]  # ~564.98
            cx = self.camera_params[2]  # ~303.59

            # Calculate the raw 3D estimates first
            raw_3d_yaw_deg = self._compute_yaw_deg(tag.pose_R)
            raw_3d_lateral_cm = math.sin(math.radians(raw_3d_yaw_deg)) * distance_cm

            # ──────────────────────────────────────────────────────────────────────
            # Accurate 2D Pinhole Fallback Calculations
            # ──────────────────────────────────────────────────────────────────────
            # 1. Use true optical center (cx) instead of arbitrary frame width / 2
            pixel_offset_x = centre_x - cx

            # 2. Calculate true bearing angle using the camera's focal length (fx)
            bearing_rad = math.atan2(pixel_offset_x, fx)
            bearing_deg = math.degrees(bearing_rad)

            # 3. Calculate true lateral offset based on bearing and distance
            lateral_2d_cm = distance_cm * math.sin(bearing_rad)

            # ──────────────────────────────────────────────────────────────────────
            # Smooth Blending (Prevents steering jerks on your 20x20cm chassis)
            # ──────────────────────────────────────────────────────────────────────
            D_CLOSE = 100.0  # cm: Below this, trust 3D pose 100% (stable)
            D_FAR   = 130.0  # cm: Above this, trust 2D fallback 100% (ambiguity zone)

            if distance_cm >= D_FAR:
                # Too far: Use pure 2D visual servoing
                yaw_deg = bearing_deg
                lateral_cm = lateral_2d_cm

            elif distance_cm <= D_CLOSE:
                # Close up: Use pure 3D pose estimation for precise docking
                yaw_deg = raw_3d_yaw_deg
                lateral_cm = raw_3d_lateral_cm

            else:
                # Transition Zone (100cm - 130cm): Interpolate smoothly between the two
                # Weight goes from 0.0 (at D_CLOSE) to 1.0 (at D_FAR)
                weight_2d = (distance_cm - D_CLOSE) / (D_FAR - D_CLOSE)
                weight_3d = 1.0 - weight_2d
                
                yaw_deg = (bearing_deg * weight_2d) + (raw_3d_yaw_deg * weight_3d)
                lateral_cm = (lateral_2d_cm * weight_2d) + (raw_3d_lateral_cm * weight_3d)
